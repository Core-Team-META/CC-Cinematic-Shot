...
function Module.Ease(object, property, goal, easeDuration, easingEquation, easingDirection)
	...
	if(property == "Rotation" or property == "WorldRotation") then
		--easeX = directionX * math.abs(goalX - startX)
		--easeY = directionY * math.abs(goalY - startY)
		--easeZ = directionZ * math.abs(goalZ - startZ)
		...
		--[[if(startX > 0 and goalX < 0) then
			easeX = directionX * math.abs(goalX - startX)
		elseif(startX < 0 and goalX > 0) then
			easeX = directionX * math.abs(goalX - startX)
		else
			easeX = directionX * math.abs(startX - goalX)
		end
		if(startY > 0 and goalY > 0) then
			easeY = directionY * math.abs(goalY - startY)
		elseif(startY < 0 and goalY > 0) then
			easeY = directionY * math.abs(goalY - startY)
		else
			easeY = directionY * math.abs(startY - goalY)
		end
		if(startZ > 0 and goalZ > 0) then
			easeZ = directionZ * math.abs(goalZ - startZ)
		elseif(startZ < 0 and goalZ > 0) then
			easeZ = directionZ * math.abs(goalZ - startZ)
		else
			easeZ = directionZ * math.abs(startZ - goalZ)
		end]]
	else
		deltaX = directionX * math.abs(goalX - startX)
		deltaY = directionY * math.abs(goalY - startY)
	...
	wrapTask(property, object, function()
		...
		local x = easingFormula(currentTime, startX, deltaX, easeDuration)
		local y = easingFormula(currentTime, startY, deltaY, easeDuration)
		local z = easingFormula(currentTime, startZ, deltaZ, easeDuration)
...
function Module.Cancel(object)
	for _, property in pairs(tasks) do
...
function Module.EaseNumber(object, property, goal, easeDuration, easingEquation, easingDirection)
	assert(Object.IsValid(object), "Attempting to ease an object that does not exist")
	assert(type(goal) == "number", "Attempting to ease to a goal that is not a number")

	if(type(easingEquation) == "nil") then easingEquation = Module.EasingEquation.LINEAR end
	if(type(easingDirection) == "nil") then easingDirection = Module.EasingDirection.INOUT end

	assert(type(easingEquation) == "number", "Attempting to ease with an invalid easing equation")
	assert(type(easingDirection) == "number", "Attempting to ease with an invalid easing direction")

	local easingFormula = EasingEquations.GetEasingEquationFormula(easingEquation, easingDirection)
	assert(easingFormula, "Attempting to ease with an invalid easing equation enum; check that you spelled the enum correctly")

	if(not tasks[property]) then
		tasks[property] = {}
	end

	local start = object[property]
	local direction = ((start < goal) and 1) or -1
	local delta = direction * math.abs(goal - start)
	...
	wrapTask(property, object, function()
		...
		object[property] = easingFormula(currentTime, start, delta, easeDuration)
	end)
end
